...
class CasinoDatabase {
    constructor() {
        this.dbKey = 'casinoData';
        this.accounts = this.loadAccounts();
    }

    loadAccounts() {
        const data = localStorage.getItem(this.dbKey);
        return data ? JSON.parse(data) : { 'test': { password: 'test', balance: 10000, totalBets: 0, totalWins: 0, biggestWin: 0, couponsUsed: [] } };
    }

    saveAccounts() {
        localStorage.setItem(this.dbKey, JSON.stringify(this.accounts));
    }
    ...
    registerUser(username, password) {
        ...
        this.saveAccounts();
        return { success: true, message: 'Account created! Now login.' };
    }
    ...
    savePlayerData(username, data) {
        if (this.accounts[username]) {
            this.accounts[username] = { ...this.accounts[username], ...data };
            this.saveAccounts();
        }
    }
    ...
    redeemCoupon(username, couponCode) {
        ...
        const account = this.accounts[username];
        if (account.couponsUsed.includes(couponCode)) {
            return { success: false, message: '🚫 This coupon has already been used!' };
        }

        account.couponsUsed.push(couponCode);
        account.balance += couponData[couponCode].bonus;
        this.saveAccounts();

        return { success: true, bonus: couponData[couponCode].bonus, message: couponData[couponCode].message };
    }
}
...
function loadUserData() {
    const userData = db.getPlayerData(currentUser);
    if (userData) {
...
function showCouponDialog() {
    ...
    if (code) {
        ...
        if (result.success) {
            balance += result.bonus;
            updateBalance();
            saveUserData();
            showMessage('slotMessage', result.message, 'win');
            triggerVictoryAnimation(result.bonus);
        } else {
...
function spinRoulette() {
    ...
    setTimeout(() => {
        ...
        checkRouletteWins(winningNumber);
        rouletteSpinning = false;
    }, 3000);
}

function checkRouletteWins(winningNumber) {
    ...
    if (totalWinAmount > 0) {
        ...
        if (totalWinAmount > biggestWin) {
            biggestWin = totalWinAmount;
        }
...
function drawKeno() {
    ...
    document.getElementById('kenoDrawBtn').disabled = true;
    ...
    const drawInterval = setInterval(() => {
        ...
        if (drawCount >= 20) {
            ...
            kenoSpinning = false;
            document.getElementById('kenoDrawBtn').disabled = false;
            setTimeout(() => {
                resetKenoPicks();
            }, 3000);
        }
